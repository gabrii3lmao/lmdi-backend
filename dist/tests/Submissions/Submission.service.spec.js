...
describe("SubmissionService", () => {
    ...
    beforeEach(() => {
        ...
        service = new SubmissionService(examRepoMock, subRepoMock);
        vi.clearAllMocks();
    });
    ...
    describe("getSubmissionsByExam (non-paginated)", () => {
        it("deve retornar submissões por examId", async () => {
            vi.mocked(subRepoMock.findByExamId).mockResolvedValue([{ _id: "sub-1" }]);
    ...
    describe("getSubmissionsByClass (non-paginated)", () => {
        it("deve retornar submissões por classId", async () => {
            vi.mocked(subRepoMock.findByClass).mockResolvedValue([{ _id: "sub-1" }]);
    ...
    describe("updateSubmission", () => {
        it("deve lançar HttpException 404 se submissão não existir", async () => {
            ...
            await expect(service.updateSubmission("not-found", { score: 10 })).rejects.toThrow(HttpException);
        });
        it("deve atualizar a submissão com sucesso", async () => {
            ...
            vi.mocked(subRepoMock.findById).mockResolvedValue(existing);
            vi.mocked(subRepoMock.update).mockResolvedValue(updated);
