...
describe("ExamService", () => {
    ...
    beforeEach(() => {
        ...
        service = new ExamService(examRepoMock, classRepoMock, submissionRepoMock, submissionServiceMock);
    });
    describe("createExam", () => {
        const examData = {
            title: "Nova Prova",
        ...
        it("deve criar exame com sucesso", async () => {
            ...
            vi.mocked(examRepoMock.create).mockResolvedValue(mockExam);
        ...
        it("deve lançar HttpException 404 se classe não existir", async () => {
            ...
            await expect(service.createExam(examData, "teacher-1")).rejects.toMatchObject({ statusCode: 404 });
        });
        it("deve lançar HttpException 403 se não for o professor da turma", async () => {
            ...
            await expect(service.createExam(examData, "teacher-1")).rejects.toMatchObject({ statusCode: 403 });
        });
    });
    describe("updateExam", () => {
        const updateData = {
        ...
        it("deve atualizar exame com sucesso", async () => {
            vi.mocked(examRepoMock.findByIdAndTeacher).mockResolvedValue(mockExam);
        ...
        it("deve reavaliar submissões se answerKey for modificada", async () => {
            ...
            vi.mocked(examRepoMock.findByIdAndTeacher)
                ...
                .mockResolvedValueOnce({ ...mockExam, ...updateData });
            vi.mocked(examRepoMock.update).mockResolvedValue({ ...mockExam, ...updateData });
            vi.mocked(submissionServiceMock.getSubmissionsByExam).mockResolvedValue(mockSubmissions);
            vi.mocked(gradeExam)
                .mockReturnValueOnce({
                score: 10,
                totalCorrect: 3,
        ...
        it("deve lançar HttpException 404 se exame não for encontrado", async () => {
            ...
            await expect(service.updateExam("not-found", updateData, "teacher-1")).rejects.toMatchObject({ statusCode: 404 });
        });
    });
    describe("deleteExam", () => {
        ...
        it("deve lançar HttpException 404 se exame não existir", async () => {
            vi.mocked(examRepoMock.findByIdAndTeacher).mockResolvedValue(null);
            await expect(service.deleteExam("not-found", "teacher-1")).rejects.toMatchObject({ statusCode: 404 });
        });
    });
    describe("getExamsByClass", () => {
        ...
        it("deve lançar HttpException 404 se turma não existir", async () => {
            vi.mocked(classRepoMock.findById).mockResolvedValue(null);
            await expect(service.getExamsByClass("not-found", "teacher-1", 1, 10)).rejects.toMatchObject({ statusCode: 404 });
        });
        it("deve lançar HttpException 403 se não for o professor da turma", async () => {
            const otherClass = { ...mockClass, teacherId: "other-teacher" };
            vi.mocked(classRepoMock.findById).mockResolvedValue(otherClass);
            await expect(service.getExamsByClass("class-1", "teacher-1", 1, 10)).rejects.toMatchObject({ statusCode: 403 });
        });
    });
    describe("deleteCascadeByExamId", () => {
        ...
        it("deve lançar HttpException 404 se exame não existir", async () => {
            vi.mocked(examRepoMock.findByIdAndTeacher).mockResolvedValue(null);
            await expect(service.deleteCascadeByExamId("not-found", "teacher-1")).rejects.toMatchObject({ statusCode: 404 });
        });
    });
});
//# sourceMappingURL=Exam.service.spec.js.map