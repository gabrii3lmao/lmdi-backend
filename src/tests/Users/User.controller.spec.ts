...
describe("UserController", () => {
  ...
  let req: Partial<Request & { user?: { id: string }; cookies?: Record<string, string> }>;
  ...
  describe("logout", () => {
    it("deve limpar cookie e retornar mensagem", async () => {
      await controller.logout(req as Request, res as Response, next as NextFunction);
  ...
  describe("resetPassword", () => {
    it("deve resetar senha e retornar 200", async () => {
      ...
      vi.mocked(mockService.resetPassword!).mockResolvedValue({ _id: "user-1" } as any);
  ...
  describe("refreshToken", () => {
    ...
    it("deve limpar cookie e retornar 401 se refresh token for inválido", async () => {
      ...
      vi.mocked(mockService.refreshAccessToken!).mockRejectedValue(
