...
describe("UserService", () => {
  ...
  describe("login", () => {
    ...
    it("deve realizar login com sucesso e retornar isVerified", async () => {
      ...
      vi.mocked(userRepoMock.findByEmail!).mockResolvedValue(user as any);
      ...
      expect(userRepoMock.updateRefreshToken).toHaveBeenCalledWith(
        "user-1",
        "refresh-token-123",
      );
  ...
  describe("forgotPassword", () => {
    it("deve criar token de reset e enviar email", async () => {
      ...
      expect(userRepoMock.setPasswordResetToken).toHaveBeenCalledWith(
        "prof@test.com",
        expect.any(String),
        expect.any(Number),
      );
  ...
  describe("resetPassword", () => {
    it("deve redefinir senha com token válido", async () => {
      vi.mocked(userRepoMock.resetPasswordByToken!).mockResolvedValue(mockUser as any);

      const result = await service.resetPassword("valid-token", "nova-senha");

      expect(userRepoMock.resetPasswordByToken).toHaveBeenCalledWith(
        "valid-token",
        "nova-senha",
      );
    ...
    it("deve lançar HttpException 400 se token for inválido", async () => {
      ...
      await expect(
        service.resetPassword("invalid-token", "nova-senha"),
  ...
  describe("refreshAccessToken", () => {
    it("deve renovar os tokens com sucesso", async () => {
      ...
      vi.mocked(userRepoMock.findById!).mockResolvedValue(user as any);
    ...
    it("deve lançar HttpException 401 se user não existir", async () => {
      vi.mocked(verifyRefreshToken).mockReturnValue({ id: "user-1" } as any);
